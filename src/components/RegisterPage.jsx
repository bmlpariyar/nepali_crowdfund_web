    ...
    try {
      ...
      if (err.response && err.response.data && err.response.data.errors) {
        ...
        if (Array.isArray(err.response.data.errors)) {
          ...
          errorMessages = Object.entries(err.response.data.errors).map(
            ([field, messages]) =>
              `${field.replace("_", " ")} ${messages.join(", ")}`
          );
        } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-100 via-white to-indigo-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-lg">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          {error && (
            <div
              className="bg-red-100  border-2 border-red-400 text-red-700 px-4 py-3 rounded relative text-sm"
              role="alert"
            >
