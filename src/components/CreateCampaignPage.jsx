...
function CreateCampaignPage() {
    ...
    const handleSubmit = async (event) => {
        ...
        if (!title || !story || !fundingGoal || !deadline || !categoryId) {
            setFormError("Please fill in all required fields: Title, Story, Goal, Deadline, and Category.");
            setIsLoading(false);
            return;
        }
    ...
    if (isLoadingCategories) {
        return (
            <div className="flex justify-center items-center min-h-[60vh]">
                <div className="text-lg font-medium text-gray-600">Loading form prerequisites...</div>
            </div>
        );
    }
        ...
        return (
            <div className="container mx-auto px-4 py-8 max-w-3xl text-center">
                <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-6" role="alert">
                    ...
                    <span className="block sm:inline ml-2">Could not load categories. {formError || "Please refresh."}</span>
                </div>
                <Link to="/campaigns" className="text-indigo-600 hover:text-indigo-800 font-medium">
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-3xl">
            ...
            <h2 className="text-3xl font-bold text-gray-800 mb-6">Create New Campaign</h2>
            ...
            <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-6 mb-8">
                <div className="mb-6">
                    ...
                    />
                    {fieldErrors.title && (
                        <p className="text-red-500 text-xs italic mt-1">{fieldErrors.title.join(', ')}</p>
                    )}
                </div>
                ...
                <div className="mb-6">
                    ...
                    />
                    {fieldErrors.story && (
                        <p className="text-red-500 text-xs italic mt-1">{fieldErrors.story.join(', ')}</p>
                    )}
                </div>
                ...
                <div className="mb-6">
                    ...
                    {fieldErrors.funding_goal && (
                        <p className="text-red-500 text-xs italic mt-1">{fieldErrors.funding_goal.join(', ')}</p>
                    )}
                </div>
                ...
                <div className="mb-6">
                    ...
                    {fieldErrors.deadline && (
                        <p className="text-red-500 text-xs italic mt-1">{fieldErrors.deadline.join(', ')}</p>
                    )}
                </div>
                ...
                <div className="mb-6">
                    ...
                    {fieldErrors.category_id && (
                        <p className="text-red-500 text-xs italic mt-1">{fieldErrors.category_id.join(', ')}</p>
                    )}
                    {fieldErrors.category && (
                        <p className="text-red-500 text-xs italic mt-1">{fieldErrors.category.join(', ')}</p>
                    )}
                </div>
                ...
                <div className="flex items-center justify-center">
                    <button
                        ...
                        className={`bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-6 rounded focus:outline-none focus:shadow-outline ${(isLoading || isLoadingCategories) ? 'opacity-60 cursor-not-allowed' : ''
                            }`}
                    >
